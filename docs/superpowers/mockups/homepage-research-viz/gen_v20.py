...
OUT = "/Users/shabnam/projects/website/.claude/worktrees/homepage-research-viz/docs/superpowers/mockups/homepage-research-viz/research-viz-lofi-v20.html"
...
TRI_R       = 6.6          # v18: 5.4
TRI_DOWN    = True         # apex points down, at the sentence
LEAD_W      = 1.0          # the leader's stroke
LEAD_GAP    = 3.0          # air between the leader's end and her cap line
...
ask_svg += '<text x="%d" y="%d" class="rv20-svgtext" font-size="%.1f" fill="%s">%s</text>\n' % (
...
# equilateral, circumradius TRI_R, centred on (NUDGE_X, mark_y).
_sgn = 1.0 if TRI_DOWN else -1.0
tri_apex_y = mark_y + _sgn * TRI_R
tri_pts = [(NUDGE_X, tri_apex_y),
           (NUDGE_X + TRI_R * math.sqrt(3.0) / 2.0, mark_y - _sgn * TRI_R / 2.0),
           (NUDGE_X - TRI_R * math.sqrt(3.0) / 2.0, mark_y - _sgn * TRI_R / 2.0)]
nudge_svg = '<path d="%s Z" fill="%s" stroke="none"/>' % (poly(tri_pts), INT_COL)
# the sentence keeps the y it had in v17/v18, so nothing else moved with the flip
SENT_Y1 = mark_y + 48
SENT_Y2 = mark_y + 62
# the leader: straight down from the apex, stopping LEAD_GAP above her cap line
lead_top = tri_apex_y
lead_bot = SENT_Y1 - SENT_FS * 0.78 - LEAD_GAP
nudge_svg += '<line x1="%d" y1="%.1f" x2="%d" y2="%.1f" stroke="%s" stroke-width="%.1f"/>' % (
    NUDGE_X, lead_top, NUDGE_X, lead_bot, INT_COL, LEAD_W)
link_svg = ''
...
# are untouched.
forecast_svg += '<text x="%d" y="%.0f" class="rv20-svgtext" font-size="%.1f" font-weight="700" fill="#111118">intervention</text>' % (FLBL_X, good_y-58, FLBL_FS1)
forecast_svg += '<text x="%d" y="%.0f" class="rv20-svgtext" font-size="%.1f" fill="rgba(17,17,24,0.7)">a higher likelihood of the desired outcome</text>' % (FLBL_X, good_y-43, FLBL_FS)
forecast_svg += '<text x="%d" y="%.0f" class="rv20-svgtext" font-size="%.1f" fill="rgba(17,17,24,0.7)">(internal state and/or behavior)</text>\n' % (FLBL_X, good_y-33, FLBL_FS)
forecast_svg += '<text x="%d" y="%.0f" class="rv20-svgtext" font-size="7.5" fill="%s">without intervention</text>\n' % (905, less_y+14, GHOST_COL)
...
now_svg += '<text x="%d" y="40" class="rv20-svgtext" font-size="9" fill="rgba(17,17,24,0.6)" text-anchor="middle">now</text>' % NOW_X
...
for (fam, lines, y) in FAM_LBL:
    for i, ln in enumerate(lines):
        mylbl_svg += '<text x="%d" y="%d" class="rv20-svgtext" font-size="%.1f" fill="%s">%s</text>\n' % (
...
for i, ln in enumerate(["a person&#39;s true internal state",
    ...
    mylbl_svg += '<text x="%d" y="%d" class="rv20-svgtext" font-size="%.1f" fill="rgba(17,17,24,0.6)">%s</text>\n' % (
...
mylbl_svg += '<text x="%d" y="%d" class="rv20-svgtext" font-size="%.1f" fill="#1a2c6b">our best guess at the internal state (triangulated from many diverse signals)</text>\n' % (
...
HTML = '''<!-- research-viz-lofi-v20 -->
<style>
  .rv20-wrap {{ --paper:#f5f2ec; --ink:#111118; --blue:#1a2c6b; font-family:'Space Mono', monospace; background:#f5f2ec; }}
  .rv20-wrap svg {{ display:block; width:100%; height:auto; }}
  .rv20-svgtext {{ font-family:'Space Mono', monospace; }}
</style>
...
<div class="rv20-wrap">
  ...
  <g transform="translate(0,{tp})">
    ...
    <g class="rv20-svgtext" fill="{int_col}" text-anchor="middle" font-weight="{sw}">
    ...
    <g class="rv20-svgtext" fill="#1a2c6b" text-anchor="start">
      {tslabel_svg}
    </g>
  </g>
  </svg>
...
print("intervention triangle centre (%d, %.1f) R=%.1f  apex(down) %.1f  base %.1f  half-width %.1f" % (
    NUDGE_X, mark_y, TRI_R, tri_apex_y, mark_y - _sgn*TRI_R/2.0, TRI_R*math.sqrt(3)/2))
print("leader %.1f -> %.1f (her cap line %.1f)" % (lead_top, lead_bot, SENT_Y1 - SENT_FS*0.78))
