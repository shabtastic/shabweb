...
OUT = "/Users/shabnam/projects/website/.claude/worktrees/homepage-research-viz/.superpowers/brainstorm/43386-1785285443/content/research-viz-lofi-v11.html"
...
for (lab, x1, l1, x2, l2, bow, grade) in WEB:
    ...
    if grade == "solid":
        ...
        mx, my = (x1+x2)/2.0, (y1+y2)/2.0
        web_svg += '<text x="%.0f" y="%.0f" class="rv11-svgtext" font-size="7" fill="rgba(17,17,24,0.45)" text-anchor="middle">?</text>\n' % (mx+3, my)
...
# tiny fork labels
forecast_svg += '<text x="%d" y="%.0f" class="rv11-svgtext" font-size="7.5" fill="rgba(17,17,24,0.7)">with the nudge</text>' % (900, good_y-30+ (good_y< ey and -2 or 0))
forecast_svg += '<text x="%d" y="%.0f" class="rv11-svgtext" font-size="7.5" fill="rgba(17,17,24,0.4)">without it</text>\n' % (905, less_y+14)
...
now_svg += '<text x="%d" y="40" class="rv11-svgtext" font-size="9" fill="rgba(17,17,24,0.6)" text-anchor="middle">now</text>' % NOW_X
...
HTML = '''<style>
  .rv11-wrap {{ --paper:#f5f2ec; --ink:#111118; --blue:#1a2c6b; font-family:'Space Mono', monospace; }}
  .rv11-wrap h2 {{ font-family:'DM Sans', sans-serif; }}
  .rv11-mockup-title {{ font-family:'DM Sans', sans-serif; font-weight:700; font-size:1.02rem; margin:0 0 .15rem; color:var(--blue); }}
  .rv11-mockup-desc {{ font-size:.82rem; line-height:1.45; margin:0; opacity:.82; max-width:66ch; }}
  .rv11-wrap .mockup-body {{ background:#f5f2ec; padding:0; }}
  .rv11-wrap .option {{ margin-bottom:1.15rem; }}
  .rv11-wrap svg {{ display:block; width:100%; height:auto; }}
  .rv11-cap {{ font-size:.72rem; letter-spacing:.06em; text-transform:uppercase; opacity:.6; margin:.55rem 0 .05rem; }}
  .rv11-svgtext {{ font-family:'Space Mono', monospace; }}
</style>

<div class="rv11-wrap">
  <h2>v11 — the prediction: state &#8594; outcome</h2>
  <p class="subtitle">The whole point of triangulating the hidden state is to predict — and move — an outcome we care about. Past &#39;now&#39;, the estimate projects a forecast (with its own widening uncertainty) toward that outcome; the nudge is what bends the forecast from a lesser result to a better one. Click to select and note anything in the terminal.</p>
    ...
    <div class="content">
      <div class="mockup">
        ...
        <div class="mockup-body">
          <svg viewBox="0 0 1040 500" role="img" aria-label="A faint true-state curve and a bold triangulated estimate over a web of proxies; at 'now' the estimate projects a dotted forecast cone into the future toward an outcome marker, with a ghost path to a lesser outcome without the nudge and the actual path to the better outcome after nudging; nested brackets cover only the observed span">
            <defs>
            ...
            <text x="70" y="52" class="rv11-svgtext" font-size="10.5" fill="#111118">many kinds of signal</text>
            <text x="150" y="{ts_lbl_y:.0f}" class="rv11-svgtext" font-size="10" fill="rgba(17,17,24,0.6)">the inner state we can&#39;t directly see</text>
            <text x="120" y="{est_lbl_y:.0f}" class="rv11-svgtext" font-size="10" fill="#1a2c6b">triangulated from many signals</text>
            <text x="452" y="150" class="rv11-svgtext" font-size="8" fill="#111118">gaze ↔ choice: strong evidence</text>
            <text x="300" y="156" class="rv11-svgtext" font-size="8" fill="rgba(17,17,24,0.5)">relationships we&#39;re still learning</text>
            <text x="1018" y="{gl1:.0f}" class="rv11-svgtext" font-size="8.5" fill="#111118" text-anchor="end">the outcome we care about —</text>
            <text x="1018" y="{gl2:.0f}" class="rv11-svgtext" font-size="8.5" fill="#111118" text-anchor="end">a choice, a habit, a creative leap</text>
            <g class="rv11-svgtext" fill="#111118" text-anchor="middle">
            ...
            <text x="70" y="70" class="rv11-svgtext" font-size="8" fill="#b0475f">heart · skin · breath · eyes</text>
            <text x="70" y="228" class="rv11-svgtext" font-size="8" fill="#a35e1c">behavior · movement · choices</text>
            ...
            <g class="rv11-svgtext" font-size="10" fill="#1a2c6b" text-anchor="end">
            ...
            <text x="60" y="484" class="rv11-svgtext" font-size="8.5" fill="rgba(17,17,24,0.55)">observed span (nested timescales) — seconds within minutes within hours within days within years &#183; the future lies beyond &#39;now&#39;</text>
          </svg>
        </div>
      </div>
      <div class="rv11-cap">how it composes</div>
      <p class="rv11-mockup-title">The prediction: state &#8594; outcome</p>
      <p class="rv11-mockup-desc">The full chain now reads left to right: many diverse signals, related in a partly-known web, triangulate the hidden inner state into the bold estimate. At &#39;now&#39; the estimate projects forward — a dotted forecast cone that widens with uncertainty — toward an outcome we actually care about: a choice, a habit that sticks, a creative leap. Two futures branch from the same state: without intervening, the ghost path drifts to a lesser outcome; the nudge, applied at the right moment, bends the trajectory to the better one. The true state itself fades at &#39;now&#39; — the future is ours to predict, not to observe.</p>
    </div>
  </div>
</div>
