...
OUT = "/Users/shabnam/projects/website/.claude/worktrees/homepage-research-viz/docs/superpowers/mockups/homepage-research-viz/research-viz-lofi-v22.html"

# ---------- v22: the intervention marker is the highlighter swipe ----------
# The one open item on the figure — "not in love with the triangle" — is closed.
# Six treatments were drawn on this same geometry in gen_marker_options.py
# (A filled triangle = v21, B purple tick in the olive ticks' grammar, C diamond,
# D open caret, E no glyph at all, F highlighter swipe). She chose F.
#
# F is ported here exactly as that file defines it: same width, same top/bottom
# derivation from the band and the grey curve, same 0.30 fill, same 1.5px hairline
# core at 0.85, same fully-rounded ends (rx = half the width). Nothing about it is
# re-tuned — the version she looked at is the version that ships.
#
# Why it works, restated so it does not get "improved" later: the site is built
# on a highlighter palette, and a swipe over an instant is the one mark this
# figure can make that needs no explaining. It is a fill, not a line and not a
# dot, so it cannot be confused with the olive individual-experience ticks; it is
# vertical like 'now', so it reads as a moment in time rather than an object; and
# it holds at any size because it is an area. The hairline core keeps it crisp.
#
# It spans the whole vertical extent of the thing being acted on — from just
# above whichever of the estimate band and the grey true-state curve runs higher,
# to just below whichever runs lower — so it does not have to choose between the
# navy and the grey the way the triangle did. That is what removes v21's problem:
# the triangle had to abut one curve and therefore looked pinned to it.
#
# No leader, no arrow, no glyph anywhere else. Her sentence stays the same purple
# directly beneath it, and stays her words, unchanged. Nothing else in v21 moves:
# warped axis, strands, rising olive ticks, both curves, the sampling band, the
# brackets, the forecast zone and the 34px title headroom are all bit-identical.
F_W        = 13.0    # swipe width in px — a marker nib, wide enough to read as a
                     # fill rather than a fat line, narrow enough to stay an instant
F_PAD_TOP  = 9.0     # px of overshoot above the higher of (band top, grey curve)
F_PAD_BOT  = 13.0    # px below the lower of the two; bigger, so the swipe leans
                     # toward her sentence without needing a leader to reach it
F_OP       = 0.30    # ink density of the swipe body
F_CORE_W   = 1.5     # the hairline core, so the mark does not go soft at the
                     # middle the way a bare translucent fill does
F_CORE_OP  = 0.85
F_CORE_IN  = 4.0     # core inset from each rounded end, so the core stops short
                     # of the caps and the ends stay round rather than blunt

# ---------- v20: the tick she circled was never a tick ----------
# She circled a mark that looked unpegged. It was: it was the legend miniature —
# a stem and droplet drawn at x=192, hovering ~42px above the grey curve with
# nothing under it. The eight real ticks measure 0.0 / 0.0 / -0.3 / 0.0 / 0.0 /
# +0.4 / -0.3 / -0.8 px against the true-state polyline; they were pegged all
# along. The legend device, introduced in v17 to tie the words to the marks, had
# turned into precisely what the README bans: a floating glyph that reads as a
# detached instance of the mark it was supposed to explain.
#
# So the miniature is deleted outright, and the words are moved into contact
# with a REAL tick instead. No glyph, no leader line — the label is simply
# nearer to that mark than to anything else on the page.
#
# Anchor: the x=346 tick, words to its LEFT, level with its droplet.
#   the x=470 tick was the obvious candidate (highest droplet, most open paper)
#   and it does not work at 9px: the corridor between the lowest strand and the
#   upper edge of the sampling band TILTS across the 113px the words need —
#   [202.0, 219.6] at x=474 but [216.9, 235.6] at x=594 — so the widest
#   horizontal slot that fits inside it everywhere is 6.6px, and the words need
#   9.2px. The x=346 tick is the only anchor with real room around it.
TICK_LBL_X  = 221          # words end at 334.4, 11.6px left of the droplet
TICK_LBL_Y  = 227          # box y 219.98..229.16 — 2.3px off the droplet's row

# ---------- v19 tuning: her four-item punch list on v18 ----------
# Still no new copy. Every string drawn is hers, verbatim, and there is still no
# title — she is writing that herself, so item 4 only reserves the room.
#  1. the ticks rise. They start on the grey true-state curve and grow UPWARD,
#     droplet at the top, like every other signal that feeds the estimate. The
#     whole strand block moves up to make room, exactly as she suggested.
#  2. "individual experience" goes to 9px, the size of her two family names.
#  3. the intervention triangle grows a little, flips apex-down, and drops a
#     leader from that apex to her purple sentence.
#  4. headroom at the top of the canvas for the title she is supplying.

# --- 1. the ticks rise, and the strands move up to let them ---
# The estimate runs ABOVE the grey curve at six of the eight tick positions, so
# an upward tick has to pass the navy line to be read as rising. Two things make
# that clean rather than muddy:
#   * paint order — the ticks are drawn before the estimate, so the navy stroke
#     overdraws each crossing rather than being interrupted by it;
#   * length — 20px of rise puts every droplet at least 8px clear of the top
#     edge of the navy stroke, in open paper, where it reads as the end of a
#     mark growing out of the grey line.
# The room for that comes from the strand block: every lane moves up 22px, which
# is the smallest shift that leaves the highest droplet (x=470, y=211.6) clear
# of the lowest strand. It also fixes an old overlap — the sampling band's upper
# edge used to climb 7px past the choices lane around x=470.
STRAND_DY   = -22          # every strand lane, up
TICK_DIR    = -1           # v18: +1 (down). The ticks rise now.
TICK_STEM   = 16.0         # v18: 9 — the stroke has further to travel
TICK_DROP   = 20.0         # v18: 12 — droplet clears the navy stroke by >=8px
CLR_STRAND  = 8.0          # required air between a droplet and the lowest strand

# --- 3. the intervention triangle, apex down, with a leader to her sentence ---
# v21: apex-UP again, 7.6% larger, and re-seated under the GREY true-state curve
# with its apex abutting that line. LEAD_W / LEAD_GAP are gone with the leader.
# v22: the triangle is gone. TRI_R went with it, and so did GREY_W — the only
# thing GREY_W ever did was let the apex sit on the grey stroke's lower edge, and
# the swipe crosses that stroke instead of abutting it. The grey curve's own
# stroke-width is still declared where it is drawn, in the fragment, as it always
# was. See the F_* constants at the top of the file.

# --- 4. headroom for her title ---
...
# Centred on the estimate at x=600. No shaft, no marker def, so there is nothing
# left that could read as an arrowhead: it is a point marker on the curve, the
# same job v16's dot did, in the same colour.
# v19: TRI_R lives at the top of the file now — bigger, and apex-down.
...
TICK_COL    = "#587722"
# v19: TICK_STEM / TICK_DROP / TICK_DIR now live at the top of the file — the
# ticks rise, and they are longer.
...
# v19: the words go to FAM_FS — the size of "measurable physiology" and
# "observable behavior" — so all three of her label families are one size.
TICK_LBL_FS = FAM_FS       # v18: 6.5
# v20: TICK_GLYPH_X is gone with the miniature, and TICK_LBL_X / TICK_LBL_Y are
# set at the top of the file, against the x=346 tick.
...
# v19: the whole block moves up by STRAND_DY to open the band the rising ticks
# need. Nothing else about the strands changes — they still bend into
# estimate(xm) at their own merge x, just over a slightly longer drop.
STRANDS = [(nm, col, y0 + STRAND_DY, damp, xs, xm, hero)
...
# v19: and it RISES. Downward ticks read as hanging off the curve rather than
# feeding anything; upward is the same gesture every other signal makes on its
# way into the estimate. The estimate is above the grey curve at six of the
# eight positions, so each of those ticks passes behind the navy stroke — this
# block is emitted before est_svg, so the estimate overdraws the crossing
# cleanly — and comes out at least 8px above it, in open paper. The strand block
# moved up 22px to give the droplets that paper.
for (dx, r) in drops:
...
# v20: the legend miniature — stem and droplet — is deleted. Nothing is drawn
# here but her words, and they sit close enough to the x=346 tick that the tick
# is the nearest thing to them on the page: 11.6px to the left of its droplet
# and 2.3px off its row. The three next-nearest marks are also ticks (x=214 at
# 15.1px, x=300 at 17.7px). No glyph, no leader line.
ask_svg += '<text x="%d" y="%d" class="rv22-svgtext" font-size="%.1f" fill="%s">%s</text>\n' % (
...
# v19: a little larger, and turned over — the apex points DOWN, and a thin
# leader in the same colour runs from that apex to her sentence. This is the one
# leader line in the figure, and she asked for it by name: the marker and the
# annotation are the same statement, so they are joined.
# v21: back to apex-UP, and off the estimate entirely — it now hangs BELOW the
# grey true-state curve with its apex touching that line. The thing the
# intervention acts on is the person's actual state, so the marker abuts the
# grey, not the navy. The leader she asked for in v19 is deleted again: with the
# triangle sitting on its own under the curve there is nothing left in the
# figure that points at anything.
# v22: no triangle at all. Treatment F from the marker comparison — the
# highlighter swipe — drawn at x=NUDGE_X as two elements and nothing else:
#   * a rounded vertical band at F_OP, spanning from F_PAD_TOP above the higher
#     of (top of the sampling band, the grey curve) to F_PAD_BOT below the lower
#     of the two, so it crosses BOTH curves instead of hanging off either one;
#   * a hairline core inset F_CORE_IN from each rounded end.
# Both are struck at x=600 with no stroke on the band, so there is no outline to
# read as a shape. Still no leader, no arrow, no glyph.
mark_y = estimate(NUDGE_X)          # still what her sentence is positioned from
grey_y = true_state(NUDGE_X)
F_TOP = min(mark_y - err_env(NUDGE_X), grey_y) - F_PAD_TOP
F_BOT = max(mark_y + err_env(NUDGE_X), grey_y) + F_PAD_BOT
nudge_svg  = '<rect x="%.1f" y="%.1f" width="%.1f" height="%.1f" rx="%.1f" fill="%s" opacity="%.2f"/>' % (
    NUDGE_X - F_W / 2, F_TOP, F_W, F_BOT - F_TOP, F_W / 2, INT_COL, F_OP)
nudge_svg += '<line x1="%d" y1="%.1f" x2="%d" y2="%.1f" stroke="%s" stroke-width="%.1f" opacity="%.2f"/>' % (
    NUDGE_X, F_TOP + F_CORE_IN, NUDGE_X, F_BOT - F_CORE_IN, INT_COL, F_CORE_W, F_CORE_OP)
# the sentence keeps the y it has had since v17, so none of this moved it
SENT_Y1 = mark_y + 48
SENT_Y2 = mark_y + 62
link_svg = ''
...
# v19: + TOP_PAD, the empty band her title will occupy. The drawing is translated
# down by exactly that much, so no y inside the figure moved.
CANVAS_H = TOP_PAD + BR_FLOOR + 24    # v18: 448, v17: 500
NOW_Y2   = BR_FLOOR + 16
...
FAM_LBL = [
    ("body",     ["measurable", "physiology"], 105 + STRAND_DY),   # lanes 56-119
    ("behavior", ["observable", "behavior"],   190 + STRAND_DY),   # lanes 151-193
]
mylbl_svg = ""
...
mylbl_svg += '<text x="%d" y="%d" class="rv22-svgtext" font-size="%.1f" fill="#1a2c6b">our best guess at the internal state (triangulated from many diverse signals)</text>\n' % (
...
HTML = '''<!-- research-viz-lofi-v22 -->
<style>
  .rv22-wrap {{ --paper:#f5f2ec; --ink:#111118; --blue:#1a2c6b; font-family:'Space Mono', monospace; background:#f5f2ec; }}
  .rv22-wrap svg {{ display:block; width:100%; height:auto; }}
  .rv22-svgtext {{ font-family:'Space Mono', monospace; }}
</style>
...
<div class="rv22-wrap">
  ...
  <!-- v19: the drawing is translated down by {tp}px. That band at the top is
       ...
       coordinate below is unchanged from v18, so the title drops in without a
  ...
  <g transform="translate(0,{tp})">
    ...
    <g class="rv22-svgtext" fill="{int_col}" text-anchor="middle" font-weight="{sw}">
    ...
    <g class="rv22-svgtext" fill="#1a2c6b" text-anchor="start">
      {tslabel_svg}
    </g>
  </g>
  </svg>
...
print("intervention swipe (treatment F): x %.1f..%.1f (w %.1f, rx %.1f) y %.3f..%.3f (h %.3f) op %.2f" % (
    NUDGE_X - F_W/2, NUDGE_X + F_W/2, F_W, F_W/2, F_TOP, F_BOT, F_BOT - F_TOP, F_OP))
print("  core: x=%d y %.3f..%.3f  width %.1f  op %.2f  (inset %.1f from each end)" % (
    NUDGE_X, F_TOP + F_CORE_IN, F_BOT - F_CORE_IN, F_CORE_W, F_CORE_OP, F_CORE_IN))
print("  grey true state at x=%d: %.3f   estimate %.3f  band %.3f..%.3f -> swipe covers both" % (
    NUDGE_X, grey_y, mark_y, mark_y - err_env(NUDGE_X), mark_y + err_env(NUDGE_X)))
print("  her cap line %.1f -> swipe foot clears it by %.1f" % (
    SENT_Y1 - SENT_FS*0.78, (SENT_Y1 - SENT_FS*0.78) - F_BOT))
print("  nearest olive ticks: %s  (swipe spans x %.1f..%.1f)" % (
    sorted(drops, key=lambda d: abs(d[0]-NUDGE_X))[:2], NUDGE_X - F_W/2, NUDGE_X + F_W/2))
...
print("canvas height %d = %d title headroom + %d drawing  bracket floors %d..%d  now divider ends %d" % (
    CANVAS_H, TOP_PAD, CANVAS_H - TOP_PAD, TS_TOP + TS_BASE, BR_FLOOR, NOW_Y2))
print("strand lanes (v18 y %s) -> %s" % (
    [y0 - STRAND_DY for (nm, c, y0, d, xs, xm, h) in STRANDS],
    [y0 for (nm, c, y0, d, xs, xm, h) in STRANDS]))
for i, (lab, xl, xr) in enumerate(TIMESCALES):
