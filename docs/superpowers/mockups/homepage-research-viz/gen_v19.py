...
OUT = "/Users/shabnam/projects/website/.claude/worktrees/homepage-research-viz/docs/superpowers/mockups/homepage-research-viz/research-viz-lofi-v19.html"
...
# v19: at 9px the block is 123px wide, which no longer fits any pocket between
# two ticks at droplet height. It moves into the corridor the strand shift just
# opened — the clear band between the lowest strand and the top of the sampling
# band — where it caps the leftmost run of ticks. Chosen by scanning every
# (x, baseline) in that corridor for the one with the most clearance on all four
# sides while staying off the true-state label's column: 10.6px, with the real
# x=214 tick 22px to the right of the miniature.
TICK_GLYPH_X = 192         # the miniature's stem
TICK_LBL_X  = 202          # words start here, 10px after the stem
# 220 rather than the scan's 216: it drops the block 4px toward the ticks it
# names and still keeps 7px off the sampling band's upper edge (14.6px off the
# lowest strand, 18.8px off the nearest droplet). The band is the hard limit
# here — below y=227 the words would be set on top of it.
TICK_LBL_Y  = 220
...
# v19: the miniature flips with the real thing — stroke rising off a baseline,
# droplet at the top — and the words go to 9px, the size of her two family names,
# so the three label families are one size. It is drawn a little taller to stay
# in proportion with the 9px text.
ask_svg += '<line x1="%d" y1="%.1f" x2="%d" y2="%.1f" stroke="%s" stroke-width="1.2" opacity="0.55"/>' % (
    TICK_GLYPH_X, TICK_LBL_Y + 0.5, TICK_GLYPH_X, TICK_LBL_Y - 5.5, TICK_COL)
ask_svg += '<circle cx="%d" cy="%.1f" r="1.6" fill="%s" opacity="0.7"/>' % (
    TICK_GLYPH_X, TICK_LBL_Y - 7.5, TICK_COL)
ask_svg += '<text x="%d" y="%d" class="rv19-svgtext" font-size="%.1f" fill="%s">%s</text>\n' % (
...
# are untouched.
forecast_svg += '<text x="%d" y="%.0f" class="rv19-svgtext" font-size="%.1f" font-weight="700" fill="#111118">intervention</text>' % (FLBL_X, good_y-58, FLBL_FS1)
forecast_svg += '<text x="%d" y="%.0f" class="rv19-svgtext" font-size="%.1f" fill="rgba(17,17,24,0.7)">a higher likelihood of the desired outcome</text>' % (FLBL_X, good_y-43, FLBL_FS)
forecast_svg += '<text x="%d" y="%.0f" class="rv19-svgtext" font-size="%.1f" fill="rgba(17,17,24,0.7)">(internal state and/or behavior)</text>\n' % (FLBL_X, good_y-33, FLBL_FS)
forecast_svg += '<text x="%d" y="%.0f" class="rv19-svgtext" font-size="7.5" fill="%s">without intervention</text>\n' % (905, less_y+14, GHOST_COL)
...
now_svg += '<text x="%d" y="40" class="rv19-svgtext" font-size="9" fill="rgba(17,17,24,0.6)" text-anchor="middle">now</text>' % NOW_X
...
for (fam, lines, y) in FAM_LBL:
    for i, ln in enumerate(lines):
        mylbl_svg += '<text x="%d" y="%d" class="rv19-svgtext" font-size="%.1f" fill="%s">%s</text>\n' % (
...
for i, ln in enumerate(["a person&#39;s true internal state",
    ...
    mylbl_svg += '<text x="%d" y="%d" class="rv19-svgtext" font-size="%.1f" fill="rgba(17,17,24,0.6)">%s</text>\n' % (
...
mylbl_svg += '<text x="%d" y="%d" class="rv19-svgtext" font-size="%.1f" fill="#1a2c6b">our best guess at the internal state (triangulated from many diverse signals)</text>\n' % (
...
HTML = '''<!-- research-viz-lofi-v19 -->
<style>
  .rv19-wrap {{ --paper:#f5f2ec; --ink:#111118; --blue:#1a2c6b; font-family:'Space Mono', monospace; background:#f5f2ec; }}
  .rv19-wrap svg {{ display:block; width:100%; height:auto; }}
  .rv19-svgtext {{ font-family:'Space Mono', monospace; }}
</style>
...
<div class="rv19-wrap">
  ...
  <g transform="translate(0,{tp})">
    ...
    <g class="rv19-svgtext" fill="{int_col}" text-anchor="middle" font-weight="{sw}">
    ...
    <g class="rv19-svgtext" fill="#1a2c6b" text-anchor="start">
      {tslabel_svg}
    </g>
  </g>
  </svg>
