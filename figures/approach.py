...
# prose left anywhere in the output is hers, inside the drawing.
#
# Production note: this is the standalone <svg>...</svg> element only — the
# page-level `.rv22-wrap` style block and wrapping <div> that the mockup used
...
# v16: write pure ASCII — the em-dashes and arrows go out as numeric entities,
# so the fragment renders the same in any shell, with or without a charset
# declaration. No wording changes; only the encoding. (Originally applied only
# at file-write time; applied here so svg_fragment() returns the same bytes
# regardless of how it is consumed.)
SVG = SVG.encode("ascii", "xmlcharrefreplace").decode("ascii")
