...
OUT = "/Users/shabnam/projects/website/.claude/worktrees/homepage-research-viz/docs/superpowers/mockups/homepage-research-viz/research-viz-lofi-v15a.html"
...
forecast_svg += '<text x="%d" y="%.0f" class="rv15a-svgtext" font-size="%.1f" fill="#111118">intervention &#8594;</text>' % (FLBL_X, good_y-50, FLBL_FS)
forecast_svg += '<text x="%d" y="%.0f" class="rv15a-svgtext" font-size="%.1f" fill="rgba(17,17,24,0.7)">a higher likelihood of the desired outcome</text>\n' % (FLBL_X, good_y-39, FLBL_FS)
forecast_svg += '<text x="%d" y="%.0f" class="rv15a-svgtext" font-size="7.5" fill="rgba(17,17,24,0.4)">without it</text>\n' % (905, less_y+14)
...
now_svg += '<text x="%d" y="40" class="rv15a-svgtext" font-size="9" fill="rgba(17,17,24,0.6)" text-anchor="middle">now</text>' % NOW_X
...
HTML = '''<style>
  .rv15a-wrap {{ --paper:#f5f2ec; --ink:#111118; --blue:#1a2c6b; font-family:'Space Mono', monospace; }}
  .rv15a-wrap h2 {{ font-family:'DM Sans', sans-serif; }}
  .rv15a-mockup-title {{ font-family:'DM Sans', sans-serif; font-weight:700; font-size:1.02rem; margin:0 0 .15rem; color:var(--blue); }}
  .rv15a-mockup-desc {{ font-size:.82rem; line-height:1.45; margin:0; opacity:.82; max-width:66ch; }}
  .rv15a-wrap .mockup-body {{ background:#f5f2ec; padding:0; }}
  .rv15a-wrap .option {{ margin-bottom:1.15rem; }}
  .rv15a-wrap svg {{ display:block; width:100%; height:auto; }}
  .rv15a-cap {{ font-size:.72rem; letter-spacing:.06em; text-transform:uppercase; opacity:.6; margin:.55rem 0 .05rem; }}
  .rv15a-svgtext {{ font-family:'Space Mono', monospace; }}
</style>

<div class="rv15a-wrap">
  <h2>v15a — bare</h2>
  <p class="subtitle">v14a with the annotation stripped out. No curve labels, no family margins, no question marks on the dotted arcs, no parity dot trios — the figure carries itself on form, plus the four bits of wording you wrote and the timescale labels. Compare against v15b, which keeps the two curve labels and the dots.</p>
      ...
      <div class="mockup">
        ...
        <div class="mockup-body">
          <svg viewBox="0 0 1040 500" role="img" aria-label="A faint true-state curve and a bold triangulated estimate over a web of proxies; at 'now' the estimate projects a dotted forecast cone into the future toward an outcome marker, with a ghost path to a lesser outcome without the nudge and the actual path to the better outcome after nudging; five nested timescale brackets — seconds, hours, days, weeks, years — each straddle the 'now' divider, reaching back into the observed signals and forward into the forecast">
            ...
            <g class="rv15a-svgtext" fill="#111118" text-anchor="middle">
            ...
            <g class="rv15a-svgtext" fill="#1a2c6b" text-anchor="start">
              {tslabel_svg}
            </g>
            <text x="60" y="484" class="rv15a-svgtext" font-size="8.5" fill="rgba(17,17,24,0.55)">the state and its signals evolve together at every timescale — how far back we&#39;ve measured, how far ahead we can see &#183; the state keeps changing even as we measure it</text>
          </svg>
        </div>
      </div>
      <div class="rv15a-cap">what changed from v14a</div>
      <p class="rv15a-mockup-title">Bare — form only</p>
      <p class="rv15a-mockup-desc">Gone: the seven per-curve annotations, the &#8220;?&#8221; glyphs on the dotted arcs, and the three parity dot trios. Kept: every arc including the heavier gaze-choice one, the solid-versus-dotted distinction, and all the geometry. Parity is still in the picture — the strands fold into the estimate on identical terms and the uncertainty band tightens as each one arrives — it is just no longer spelled out in dots. The lanes moved 3px further apart to use the room the margin labels had been holding.</p>
