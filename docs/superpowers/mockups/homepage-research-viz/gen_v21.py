...
OUT = "/Users/shabnam/projects/website/.claude/worktrees/homepage-research-viz/docs/superpowers/mockups/homepage-research-viz/research-viz-lofi-v21.html"
...
TRI_R       = 7.1          # v20: 6.6, v18: 5.4  (+7.6%)
GREY_W      = 2.0          # the true-state curve's stroke width, so the apex can
                           # sit exactly on its lower edge rather than inside it
...
ask_svg += '<text x="%d" y="%d" class="rv21-svgtext" font-size="%.1f" fill="%s">%s</text>\n' % (
...
# apex sits on the grey stroke's lower edge, so the two shapes touch and neither
# is drawn over the other.
tri_apex_y = grey_y + GREY_W / 2.0
_hw = TRI_R * math.sqrt(3.0) / 2.0  # half the base
_h  = TRI_R * 1.5                   # apex-to-base height of an equilateral
tri_pts = [(NUDGE_X, tri_apex_y),
           (NUDGE_X + _hw, tri_apex_y + _h),
           (NUDGE_X - _hw, tri_apex_y + _h)]
...
# are untouched.
forecast_svg += '<text x="%d" y="%.0f" class="rv21-svgtext" font-size="%.1f" font-weight="700" fill="#111118">intervention</text>' % (FLBL_X, good_y-58, FLBL_FS1)
forecast_svg += '<text x="%d" y="%.0f" class="rv21-svgtext" font-size="%.1f" fill="rgba(17,17,24,0.7)">a higher likelihood of the desired outcome</text>' % (FLBL_X, good_y-43, FLBL_FS)
forecast_svg += '<text x="%d" y="%.0f" class="rv21-svgtext" font-size="%.1f" fill="rgba(17,17,24,0.7)">(internal state and/or behavior)</text>\n' % (FLBL_X, good_y-33, FLBL_FS)
forecast_svg += '<text x="%d" y="%.0f" class="rv21-svgtext" font-size="7.5" fill="%s">without intervention</text>\n' % (905, less_y+14, GHOST_COL)
...
now_svg += '<text x="%d" y="40" class="rv21-svgtext" font-size="9" fill="rgba(17,17,24,0.6)" text-anchor="middle">now</text>' % NOW_X
...
for (fam, lines, y) in FAM_LBL:
    for i, ln in enumerate(lines):
        mylbl_svg += '<text x="%d" y="%d" class="rv21-svgtext" font-size="%.1f" fill="%s">%s</text>\n' % (
...
for i, ln in enumerate(["a person&#39;s true internal state",
    ...
    mylbl_svg += '<text x="%d" y="%d" class="rv21-svgtext" font-size="%.1f" fill="rgba(17,17,24,0.6)">%s</text>\n' % (
...
mylbl_svg += '<text x="%d" y="%d" class="rv21-svgtext" font-size="%.1f" fill="#1a2c6b">our best guess at the internal state (triangulated from many diverse signals)</text>\n' % (
...
HTML = '''<!-- research-viz-lofi-v21 -->
<style>
  .rv21-wrap {{ --paper:#f5f2ec; --ink:#111118; --blue:#1a2c6b; font-family:'Space Mono', monospace; background:#f5f2ec; }}
  .rv21-wrap svg {{ display:block; width:100%; height:auto; }}
  .rv21-svgtext {{ font-family:'Space Mono', monospace; }}
</style>
...
<div class="rv21-wrap">
  ...
  <g transform="translate(0,{tp})">
    ...
    <g class="rv21-svgtext" fill="{int_col}" text-anchor="middle" font-weight="{sw}">
    ...
    <g class="rv21-svgtext" fill="#1a2c6b" text-anchor="start">
      {tslabel_svg}
    </g>
  </g>
  </svg>
...
print("intervention triangle R=%.1f apex-UP at (%d, %.1f)  base y %.1f  half-width %.1f" % (
    TRI_R, NUDGE_X, tri_apex_y, tri_apex_y + _h, _hw))
print("  grey true state at x=%d: %.3f  (stroke %.1f -> lower edge %.3f)  apex gap %.3f" % (
    NUDGE_X, grey_y, GREY_W, grey_y + GREY_W/2.0, tri_apex_y - (grey_y + GREY_W/2.0)))
print("  estimate at x=%d: %.1f  (stroke bottom %.1f) -> triangle clears the navy by %.1f" % (
    NUDGE_X, mark_y, mark_y + 4.8/2, tri_apex_y - (mark_y + 4.8/2)))
print("  her cap line %.1f -> triangle base clears it by %.1f" % (
    SENT_Y1 - SENT_FS*0.78, (SENT_Y1 - SENT_FS*0.78) - (tri_apex_y + _h)))
