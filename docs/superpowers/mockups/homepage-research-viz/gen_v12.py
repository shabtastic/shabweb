...
OUT = "/Users/shabnam/projects/website/.claude/worktrees/homepage-research-viz/docs/superpowers/mockups/homepage-research-viz/research-viz-lofi-v12.html"
...
for (lab, x1, l1, x2, l2, bow, grade) in WEB:
    ...
    if grade == "solid":
        ...
        mx, my = (x1+x2)/2.0, (y1+y2)/2.0
        web_svg += '<text x="%.0f" y="%.0f" class="rv12-svgtext" font-size="7" fill="rgba(17,17,24,0.45)" text-anchor="middle">?</text>\n' % (mx+3, my)
...
# ---------- nudge (on the true state, informed by the estimate) ----------
tsn = true_state(NUDGE_X)
nudge_svg = '<line x1="%d" y1="%.1f" x2="%d" y2="%.1f" stroke="#FF7792" stroke-width="3" marker-end="url(#nud11)"/>' % (NUDGE_X, tsn+50, NUDGE_X, tsn+4)
...
# tiny fork labels
forecast_svg += '<text x="%d" y="%.0f" class="rv12-svgtext" font-size="7.5" fill="rgba(17,17,24,0.7)">with the nudge</text>' % (900, good_y-30+ (good_y< ey and -2 or 0))
forecast_svg += '<text x="%d" y="%.0f" class="rv12-svgtext" font-size="7.5" fill="rgba(17,17,24,0.4)">without it</text>\n' % (905, less_y+14)
...
now_svg += '<text x="%d" y="40" class="rv12-svgtext" font-size="9" fill="rgba(17,17,24,0.6)" text-anchor="middle">now</text>' % NOW_X
...
HTML = '''<style>
  .rv12-wrap {{ --paper:#f5f2ec; --ink:#111118; --blue:#1a2c6b; font-family:'Space Mono', monospace; }}
  .rv12-wrap h2 {{ font-family:'DM Sans', sans-serif; }}
  .rv12-mockup-title {{ font-family:'DM Sans', sans-serif; font-weight:700; font-size:1.02rem; margin:0 0 .15rem; color:var(--blue); }}
  .rv12-mockup-desc {{ font-size:.82rem; line-height:1.45; margin:0; opacity:.82; max-width:66ch; }}
  .rv12-wrap .mockup-body {{ background:#f5f2ec; padding:0; }}
  .rv12-wrap .option {{ margin-bottom:1.15rem; }}
  .rv12-wrap svg {{ display:block; width:100%; height:auto; }}
  .rv12-cap {{ font-size:.72rem; letter-spacing:.06em; text-transform:uppercase; opacity:.6; margin:.55rem 0 .05rem; }}
  .rv12-svgtext {{ font-family:'Space Mono', monospace; }}
</style>

<div class="rv12-wrap">
  <h2>v12 — timescales that straddle &#39;now&#39;</h2>
  <p class="subtitle">Same figure as v11, one change: the timescale brackets no longer stop at &#39;now&#39;. Each one is centred on the &#39;now&#39; divider — its left arm is the window of signal it draws on, its right arm how far ahead a prediction at that rate can see. seconds hugs &#39;now&#39;; years spans nearly the whole observed record and reaches the outcome marker. Click to select and note anything in the terminal.</p>
        ...
        <div class="mockup-body">
          <svg viewBox="0 0 1040 500" role="img" aria-label="A faint true-state curve and a bold triangulated estimate over a web of proxies; at 'now' the estimate projects a dotted forecast cone into the future toward an outcome marker, with a ghost path to a lesser outcome without the nudge and the actual path to the better outcome after nudging; five nested timescale brackets — seconds, hours, days, weeks, years — each straddle the 'now' divider, reaching back into the observed signals and forward into the forecast">
            <defs>
              ...
              <marker id="nud11" markerWidth="9" markerHeight="9" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6" fill="none" stroke="#FF7792" stroke-width="1.6"/></marker>
            </defs>
            ...
            <text x="70" y="52" class="rv12-svgtext" font-size="10.5" fill="#111118">many kinds of signal</text>
            <text x="150" y="{ts_lbl_y:.0f}" class="rv12-svgtext" font-size="10" fill="rgba(17,17,24,0.6)">the inner state we can&#39;t directly see</text>
            <text x="120" y="{est_lbl_y:.0f}" class="rv12-svgtext" font-size="10" fill="#1a2c6b">triangulated from many signals</text>
            <text x="452" y="150" class="rv12-svgtext" font-size="8" fill="#111118">gaze ↔ choice: strong evidence</text>
            <text x="300" y="156" class="rv12-svgtext" font-size="8" fill="rgba(17,17,24,0.5)">relationships we&#39;re still learning</text>
            <text x="1018" y="{gl1:.0f}" class="rv12-svgtext" font-size="8.5" fill="#111118" text-anchor="end">the outcome we care about —</text>
            <text x="1018" y="{gl2:.0f}" class="rv12-svgtext" font-size="8.5" fill="#111118" text-anchor="end">a choice, a habit, a creative leap</text>
            <g class="rv12-svgtext" fill="#111118" text-anchor="middle">
            ...
            <text x="70" y="70" class="rv12-svgtext" font-size="8" fill="#b0475f">heart · skin · breath · eyes</text>
            <text x="70" y="228" class="rv12-svgtext" font-size="8" fill="#a35e1c">behavior · movement · choices</text>

            <g fill="none" stroke="#1a2c6b" stroke-width="1.3">
              {bracket_svg}
            </g>
            <g class="rv12-svgtext" fill="#1a2c6b" text-anchor="start">
              {tslabel_svg}
            </g>
            <text x="60" y="484" class="rv12-svgtext" font-size="8.5" fill="rgba(17,17,24,0.55)">every timescale straddles &#39;now&#39; — how far back the signals reach at that rate, and how far ahead it lets us see &#183; seconds within hours within days within weeks within years</text>
          </svg>
        </div>
      </div>
      <div class="rv12-cap">how it composes</div>
      <p class="rv12-mockup-title">The prediction: state &#8594; outcome</p>
      <p class="rv12-mockup-desc">The full chain now reads left to right: many diverse signals, related in a partly-known web, triangulate the hidden inner state into the bold estimate. At &#39;now&#39; the estimate projects forward — a dotted forecast cone that widens with uncertainty — toward an outcome we actually care about: a choice, a habit that sticks, a creative leap. Two futures branch from the same state: without intervening, the ghost path drifts to a lesser outcome; the nudge, applied at the right moment, bends the trajectory to the better one. The true state itself fades at &#39;now&#39; — the future is ours to predict, not to observe. Underneath, the timescales are drawn as nested brackets centred on &#39;now&#39; rather than boxes over the past: most of the signal lives at seconds, hours and days, and each rate both looks back over its own window and buys a matching reach into the future — seconds for right now, years for the long horizon.</p>
    </div>
  </div>
</div>
'''.format(band_outer=band_outer, band_inner=band_inner, ts_d=ts_d,
           ...
           ts_lbl_y=ts_lbl_y, est_lbl_y=est_lbl_y, gl1=gl1, gl2=gl2, nl1=tsn+62, nl2=tsn+75)
