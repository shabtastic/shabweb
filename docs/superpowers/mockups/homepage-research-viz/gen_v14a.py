...
OUT = "/Users/shabnam/projects/website/.claude/worktrees/homepage-research-viz/docs/superpowers/mockups/homepage-research-viz/research-viz-lofi-v14a.html"
...
STRANDS = [
    ("heart",   "#FF7792",  86, 12, 70, 820, False),
    ("eyes",    "#FF7792", 104,  6, 70, 842, True),
    ("skin",    "#FF7792", 122,  8, 70, 768, False),
    ("breath",  "#FF7792", 140,  7, 70, 796, False),
    ("behavior","#FFAE77", 172,  9, 70, 690, False),
    ("movement","#FFAE77", 190,  8, 70, 730, False),
    ("choices", "#FFAE77", 208,  9, 70, 640, False),
]
...
for (lab, x1, l1, x2, l2, bow, grade) in WEB:
    ...
    if grade == "solid":
        ...
        mx, my = (x1+x2)/2.0, (y1+y2)/2.0
        web_svg += '<text x="%.0f" y="%.0f" class="rv14a-svgtext" font-size="7" fill="rgba(17,17,24,0.45)" text-anchor="middle">?</text>\n' % (mx+3, my)
...
forecast_svg += '<text x="%d" y="%.0f" class="rv14a-svgtext" font-size="%.1f" fill="#111118">intervention &#8594;</text>' % (FLBL_X, good_y-50, FLBL_FS)
forecast_svg += '<text x="%d" y="%.0f" class="rv14a-svgtext" font-size="%.1f" fill="rgba(17,17,24,0.7)">a higher likelihood of the desired outcome</text>\n' % (FLBL_X, good_y-39, FLBL_FS)
forecast_svg += '<text x="%d" y="%.0f" class="rv14a-svgtext" font-size="7.5" fill="rgba(17,17,24,0.4)">without it</text>\n' % (905, less_y+14)
...
now_svg += '<text x="%d" y="40" class="rv14a-svgtext" font-size="9" fill="rgba(17,17,24,0.6)" text-anchor="middle">now</text>' % NOW_X
...
HTML = '''<style>
  .rv14a-wrap {{ --paper:#f5f2ec; --ink:#111118; --blue:#1a2c6b; font-family:'Space Mono', monospace; }}
  .rv14a-wrap h2 {{ font-family:'DM Sans', sans-serif; }}
  .rv14a-mockup-title {{ font-family:'DM Sans', sans-serif; font-weight:700; font-size:1.02rem; margin:0 0 .15rem; color:var(--blue); }}
  .rv14a-mockup-desc {{ font-size:.82rem; line-height:1.45; margin:0; opacity:.82; max-width:66ch; }}
  .rv14a-wrap .mockup-body {{ background:#f5f2ec; padding:0; }}
  .rv14a-wrap .option {{ margin-bottom:1.15rem; }}
  .rv14a-wrap svg {{ display:block; width:100%; height:auto; }}
  .rv14a-cap {{ font-size:.72rem; letter-spacing:.06em; text-transform:uppercase; opacity:.6; margin:.55rem 0 .05rem; }}
  .rv14a-svgtext {{ font-family:'Space Mono', monospace; }}
</style>

<div class="rv14a-wrap">
  <h2>v14a — warped time axis</h2>
  <p class="subtitle">Fix A for the strands-vs-brackets mismatch. One shared log-like time axis: the observed span is one year, stretched near &#39;now&#39; and compressed toward the left edge. Every strand still runs the full width, but its fine structure is only drawn where the axis can still resolve it — heartbeats and gaze darts appear only in the last stretch, everything further left thins into a faint envelope of structure too compressed to draw. The brackets land where their units actually fall. All wording is unchanged from v13. Click to select and note anything in the terminal.</p>
      ...
      <div class="mockup">
        ...
        <div class="mockup-body">
          <svg viewBox="0 0 1040 500" role="img" aria-label="A faint true-state curve and a bold triangulated estimate over a web of proxies; at 'now' the estimate projects a dotted forecast cone into the future toward an outcome marker, with a ghost path to a lesser outcome without the nudge and the actual path to the better outcome after nudging; five nested timescale brackets — seconds, hours, days, weeks, years — each straddle the 'now' divider, reaching back into the observed signals and forward into the forecast">
            ...
            <text x="70" y="52" class="rv14a-svgtext" font-size="10.5" fill="#111118">many kinds of signal</text>
            <text x="150" y="{ts_lbl_y:.0f}" class="rv14a-svgtext" font-size="10" fill="rgba(17,17,24,0.6)">the inner state we can&#39;t directly see</text>
            <text x="120" y="{est_lbl_y:.0f}" class="rv14a-svgtext" font-size="10" fill="#1a2c6b">triangulated from many signals</text>
            <text x="452" y="150" class="rv14a-svgtext" font-size="8" fill="#111118">gaze ↔ choice: strong evidence</text>
            <text x="300" y="156" class="rv14a-svgtext" font-size="8" fill="rgba(17,17,24,0.5)">relationships we&#39;re still learning</text>
            <g class="rv14a-svgtext" fill="#111118" text-anchor="middle">
            ...
            <text x="70" y="70" class="rv14a-svgtext" font-size="8" fill="#b0475f">heart · skin · breath · eyes</text>
            <text x="70" y="228" class="rv14a-svgtext" font-size="8" fill="#a35e1c">behavior · movement · choices</text>

            <g fill="none" stroke="#1a2c6b" stroke-width="1.3">
              {bracket_svg}
            </g>
            <g class="rv14a-svgtext" fill="#1a2c6b" text-anchor="start">
              {tslabel_svg}
            </g>
            <text x="60" y="484" class="rv14a-svgtext" font-size="8.5" fill="rgba(17,17,24,0.55)">the state and its signals evolve together at every timescale — how far back we&#39;ve measured, how far ahead we can see &#183; the state keeps changing even as we measure it</text>
          </svg>
        </div>
      </div>
      <div class="rv14a-cap">what changed from v13</div>
      <p class="rv14a-mockup-title">Fix A — one warped time axis</p>
      <p class="rv14a-mockup-desc">The observed span is now a real, shared, nonlinear time axis: one year wide, logarithmic, stretched near &#39;now&#39;. Each signal has a period in seconds, and its fine structure is drawn only where a cycle is still worth more than a pixel or two — so detail switches on in a staircase that mirrors the brackets: choices first, then behaviour, skin, movement, breath, heartbeats, and gaze darts last, in the final stretch before &#39;now&#39;. To the left of each switch-on point the strand carries a faint band instead: the structure is there, the axis just cannot draw it. The fold into the estimate is re-ordered by timescale (slowest first) so each strand stays on its lane long enough to show that detail, and the estimate inherits texture only from what was resolved. Everything else — web, junctions, curves, band, cone, ghost paths, and every word of copy — is v13&#39;s.</p>
    </div>
  </div>
</div>
