#!/usr/bin/env python3
"""
gen_area_graph_mytext.py — treatment A (the area-level graph) carrying HER text.

Shabnam chose treatment A out of gen_area_graph.py: eight area nodes, edges
...
assert, same darken_to_contrast() color policy, imported from gen_area_graph.py
...
The open question is tags, which is why this file now renders D twice:

  D-tags        title + her .research-tag chips at rest
  D-tags/open   the same layout with one area's description open
  D-plain       title only at rest
  D-plain/open  the same layout with the same area's description open

Both open frames show the *longest* of her eight descriptions, so the two
variants are judged on identical content and on the hardest case for the
geometry.

Each variant's layout is solved ONCE and both of its frames are drawn from those
coordinates, so nothing moves when an area opens. The footprint reserved is
every area's open card, not just the one the mockup happens to open — clicking
any of the eight reflows nothing. Because cards grow outward, that reserve lands
in the outer margin and the titles still hug the ring. The disclosure follows the
site's existing pattern from projects.html: a chevron plus aria-expanded,
flipping when open.

Layout is solved here in Python and baked as literal SVG coordinates: no
simulation, no animation, nothing computed at load. The node circle is identical
in both variants (same centre-x, same R, same node radii, same edges); only the
canvas changes, to give the reserved cards somewhere to go.

Label placement is a box solver (solve_labels): each label block gets an
axis-aligned box, seeded radially outward from its node, then relaxed against the
eight node discs, against every other box, and against the canvas edges, with a
weak spring back to its radial seed so a label never drifts away from the node it
names. It ends with an assert that no two boxes overlap and no box sits on a node.
...
edits to index.html carry straight through. The only added glyphs are the plate
labels and the disclosure chevron. The composite's title block is empty on
purpose: the section title is hers to write.
"""

import os, re, math, html, collections

import gen_area_graph as base  # graph data, color policy, circle solver, v22 lift

HERE = os.path.dirname(os.path.abspath(__file__))
INDEX_PATH = os.environ.get(
    "WEBSITE_INDEX", "/Users/shabnam/projects/website/index.html"
)
OUT_PATH = os.path.join(HERE, "area-graph-mytext.html")
...
# ------------------------------------------------------------------- composite
def composite(recommended):
    """Reserved EMPTY title block, the approach figure <use>d once, the plate.

    The figure is lifted verbatim from research-viz-lofi-v22.html and used whole:
    per the design spec it appears exactly once and is never scoped, cropped or
    highlighted on behalf of an area.
    """
    return f"""
  <div class="composite">
...
# The area whose card the "open" frames show: the longest description, i.e. the
# hardest case for the geometry, and the same one in both variants so the two
# are compared on identical content.
D_OPEN = max(ORDER, key=lambda c: len(AREAS[c]["desc"]))
RECOMMENDED = "D-plain"

PLATE_LABELS = [
    ("D-tags", True, frozenset(), "dt1"),
    ("D-tags / open", True, {D_OPEN}, "dt2"),
    ("D-plain", False, frozenset(), "dp1"),
    ("D-plain / open", False, {D_OPEN}, "dp2"),
]


def main():
    style_v22, symbol_v22 = base.lift_v22_symbol()
    variants = {
        True: solve_variant(True, "D-tags"),
        False: solve_variant(False, "D-plain"),
    }
    frames = {
        label: d_frame(variants[tags], pfx, opened)
        for label, tags, opened, pfx in PLATE_LABELS
    }
    parts = [
        "<!doctype html>",
        "<!-- area-graph-mytext — generated by gen_area_graph_mytext.py, do not hand-edit -->",
        '<html lang="en"><head><meta charset="utf-8">',
        '<meta name="viewport" content="width=device-width, initial-scale=1">',
        "<title>area-graph-mytext</title>",
        ...
        f"""<style>
  :root {{
    --paper:{PAPER}; --ink:{INK}; --ink-blue:{INK_BLUE}; --ink-faint:rgba(17,17,24,0.15);
  ...
  .composite svg {{ display:block; width:100%; height:auto; }}
    ...
    for label, _, _, _ in PLATE_LABELS:
        parts.append(
            f'<div class="plate"><div class="plate-letter">{esc(label)}</div>'
            f'<div class="frame">{frames[label]}</div></div>'
        )
    parts += [
        '<hr class="sep"/>',
        composite(frames[RECOMMENDED]),
        "</div>",
        "</body></html>",
    ]
    open(OUT_PATH, "w").write("\n".join(parts) + "\n")

    # ---- verification: the two frames of a variant must be pixel-identical
    # everywhere except the card, so report the largest movement of any node
    # disc or any title block between the resting and the open frame.
    print("wrote", OUT_PATH)
    print("circle order:", ORDER)
    print("pairs kept:", len(KEPT), "of", len(pair_w), "at MIN_PAIR =", MIN_PAIR)
    print(
        f"opens cluster {D_OPEN} ({AREAS[D_OPEN]['title']}, "
        f"{len(AREAS[D_OPEN]['desc'])} chars — the longest)"
    )
    for tags in (True, False):
        v = variants[tags]
        name = "D-tags " if tags else "D-plain"
        shift = 0.0
        for cid in ORDER:  # same solved dict feeds both frames, by construction
            shift = max(
                shift,
                abs(v["heads"][cid][0] - (v["boxes"][cid][0] + v["geoms"][cid]["head_off"][0])),
                abs(v["heads"][cid][1] - (v["boxes"][cid][1] + v["geoms"][cid]["head_off"][1])),
            )
        wr = sorted({v["geoms"][c]["dwrap"] for c in ORDER})
        print(
            f"  {name}: canvas {W}x{v['H']:.0f}  desc measure {wr} chars  "
            f"head/disc shift between frames {shift:.2f}px  "
            f"max drift off radial seed {v['drift']:.1f}px"
        )
    print("composite carries", RECOMMENDED)
